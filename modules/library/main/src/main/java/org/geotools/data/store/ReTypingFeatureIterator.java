            ...
            if (!origAttrib.equals(attrib)) {
                if (origAttrib instanceof GeometryDescriptor) {
                    if (!(attrib instanceof GeometryDescriptor)) {
                        throw new IllegalArgumentException("Unable to retype "
                    ...
                    if (origCrs != null
                            && !origCrs.equals(((GeometryDescriptor) attrib).getCoordinateReferenceSystem())) {
                        throw new IllegalArgumentException(
                                "Unable to retype " + original.getName() + " (target have a different crs).");
                    }
                }
                if (origAttrib == null
                        || !attrib.getType()
                    ...
                    throw new IllegalArgumentException(
                            "Unable to retype " + original.getName() + " (original does not cover " + xpath + ")");
                }
            }
        }
